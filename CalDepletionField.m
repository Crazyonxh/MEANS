function filedEx=CalDepletionField(parameters,reflect)
%UNTITLED5 Summary of this function goes here
lambda1=parameters.lambda2; %nm
NA=parameters.NA;
...
if reflect==0
    for i=(1:(2*zSize+1))
     ...
     Koi = -2*pi*1i/(lambda1)*exp(1i*u(i,j)/(4*(sin(alpha/2)^2)));
      intgrand = @(theta) (sqrt(cos(theta))) .*  (sin(theta)) .*   (exp((-1i*u(i,j)/2)* (sin(theta/2).^2) / (sin(alpha/2)^2)))  .*  (besselj(1, sin(theta)/sin(alpha).*v(i,j))) ;%Min Gu P149
...
if reflect==1
    for i=(1:(2*zSize+1))
     ...
     Koi = -2*pi*1i/(lambda1)*exp(1i*u(i,j)/(4*(sin(alpha/2)^2)));
      intgrand = @(theta) (sqrt(cos(theta))) .*  (sin(theta)) .*   (exp((-1i*u(i,j)/2)* (sin(theta/2).^2) / (sin(alpha/2)^2)))  .*  (besselj(1, sin(theta)/sin(alpha).*v(i,j))).* phaseShiftCalculation( n1,n3,theta) ;%Min Gu P149
      I0 = integral(@(theta)intgrand (theta),0,alpha);  
      filedEx(i,j)=Koi.*I0;
          end;
       end;
